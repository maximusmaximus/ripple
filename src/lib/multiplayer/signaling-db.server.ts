/**
 * Postgres-backed signaling, loaded only when DATABASE_URL is set (deployed Neon).
 * Kept in a separate module so the Vercel preview bundle never evaluates `@/lib/db`
 * (which would try to boot PGLite and crash without the wasm asset).
 */
import { z } from "zod";
...
export async function handleDbSignaling(request: Request): Promise<Response> {
