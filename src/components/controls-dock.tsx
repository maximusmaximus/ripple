...
import {
  DOCK_SECTIONS,
  nextDockSection,
  readVisibleDockSection,
  type DockSectionId,
} from "@/lib/ripple/dock-nav";
...
export function ControlsDock({
  ...
  const [cursor, setCursor] = useState<DockSectionId>(DOCK_SECTIONS[0]);
  ...
  const syncCursor = () => {
    ...
    if (!el) return;
    setCursor(readVisibleDockSection(el));
  };

  useEffect(() => {
    if (!dockOpen) {
      setFocus(null);
      setCursor(DOCK_SECTIONS[0]);
      return;
    }
    const el = scrollRef.current;
    if (!el) return;
  ...
  const step = (dir: -1 | 1) => {
    const el = scrollRef.current;
    const current = el ? readVisibleDockSection(el) : cursor;
    const next = nextDockSection(current, dir);
    if (!next) return;
    setFocus(next);
    setCursor(next);
  };

  const canUp = nextDockSection(cursor, -1) != null;
  const canDown = nextDockSection(cursor, 1) != null;

  return (
    <div
      className="dock-cluster"
      data-dock-cluster="true"
      data-dock-cursor={cursor}
      data-ui-chrome
    >
      <div
      ...
      >
        <div className="relative min-h-0 flex-1 overflow-hidden">
          ...
          >
            ...
            <StudioCredit />
            <div className="dock-scroll-end" aria-hidden />
            </div>
          </div>
        </div>
      </div>
      <DockStepper onUp={() => step(-1)} onDown={() => step(1)} canUp={canUp} canDown={canDown} />
    </div>
  );
}
