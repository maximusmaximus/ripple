...
export function ColorRangeSlider() {
  ...
  const onTrackPointerDown = (e: React.PointerEvent) => {
    ...
    if (Math.abs(t - start) <= Math.abs(t - end)) {
      setColorRange({ start: t, end }); dragging.current = 'start'
    } else {
      setColorRange({ start, end: t }); dragging.current = 'end'
    }
  }
  ...
  return (
    <div className="flex flex-col gap-2 select-none" key={worldId}>
      <div className="flex items-center justify-between text-[11px] uppercase tracking-wider text-white/60">
        ...
        <button type="button" onClick={resetColorRange} disabled={isDefault}
          className="text-[10px] text-white/40 hover:text-white/80 disabled:opacity-25">Reset</button>
      </div>
      ...
      >
        <div className="pointer-events-none absolute inset-y-0 left-0 rounded-l-full bg-black/55" style={{ width: `${start * 100}%` }} />
        <div className="pointer-events-none absolute inset-y-0 right-0 rounded-r-full bg-black/55" style={{ width: `${(1 - end) * 100}%` }} />
        <div className="pointer-events-none absolute inset-y-0 border-y-2 border-white/35"
          style={{ left: `${start * 100}%`, width: `${Math.max(2, (end - start) * 100)}%` }} />

        <div data-handle="start" onPointerDown={beginDrag('start')}
          className="absolute top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 cursor-grab rounded-full border-[3px] border-white active:scale-110"
          style={{
            left: `${start * 100}%`, width: 28, height: 28, backgroundColor: startColor,
            boxShadow: `0 0 0 3px ${startColor}55, 0 2px 8px rgba(0,0,0,0.55)`,
          }}
        />
        <div data-handle="end" onPointerDown={beginDrag('end')}
          className="absolute top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 cursor-grab rounded-full border-[3px] border-white active:scale-110"
          style={{
            left: `${end * 100}%`, width: 28, height: 28, backgroundColor: endColor,
      ...
      <div className="flex justify-between font-mono text-[10px] tabular-nums text-white/45">
        ...
        <span className="text-white/30">{palette.name} · {((end - start) * 100) | 0}% window</span>
